...
ref_spat_tempo2$temporal_scale <- factor(ref_spat_tempo2$temporal_scale, 
                                     levels = c("0.5h", "1h", "3h", "6h", "12h", 
                                                "daily",  "monthly",  "seasonal", "annual"))
subset_tempo <- ref_spat_tempo2[, .(count = .N), by = .( grid_scale, temporal_scale, continent)]
...
df.grobs <-  df_plot%>% 
  do(subplots = ggplot(., aes(1, count, fill = continent)) + 
       ...
       #scale_fill_manual(labels = c("Africa", "Asia", "Europe", "North America",
                                   # "South America", "Global"), values=colset_bright2) + 
...
final_plot <- df.grobs %>%
  {ggplot(data = ., aes(factor(grid_scale), factor(temporal_scale))) + 
      ...
      scale_x_discrete("Continent", labels = c("1" = "0.1", "2" = "0.25", "3" = "0.5", 
                                               "4" = "1", "5" = "2", "6" = "2.5", "7" = "3")) +                                         #"4" = "North America", "5" = "South America", "6" = "Global")) + 
      ...
      geom_col(data = df_plot,
               ...
               colour = "white")}
      #scale_fill_manual("Spatial scale", labels = c("Africa", "Asia", "Europe", "North America",
                                                                                 #"South America", "Global"), values=colset_bright2)}
...
df.grobs <-  df_plot%>% 
  do(subplots = ggplot(., aes(1, count, fill = continent)) + 
       geom_col(position = "fill", alpha = 0.75, colour = "white") + 
       coord_polar(theta = "y") + 
       scale_fill_manual(labels = c("AFr", "Asi", "Eur", "North", "South", "Golab"), 
                         values=mycol_gridscale7) + 
       theme_void()+ guides(fill = F)) %>% 
  mutate(subgrobs = list(annotation_custom(ggplotGrob(subplots),
                                           x = grid_scale-10/17, y = temporal_scale-10/17, 
                                           xmax = grid_scale+10/17, ymax = temporal_scale+10/17))) #size of the pie charts
...
final_plot <- df.grobs %>%
  {ggplot(data = ., aes(factor(grid_scale), factor(temporal_scale))) + 
      ...
      # labels=c("0-12","13-24","25-36","37-48","49-60"), limits=c(0.5,6)) + 
      #scale_x_discrete("Continent", labels = c("1" = "Africa", "2" = "Asia", "3" = "Europe", 
      scale_x_discrete("Sptail scale", labels = c("1" = "0.1", "2" = "0.25", "3"  = "0.5", 
                                                  "4"= "1", "5" = "2",
                                                  "6" = "2.5",  "7" = "3")) + 
      scale_y_discrete("Temporal scale", labels = c("1" = "0.5h", "2" = "1h", "3" = "3h",
                                                    "4" = "6h", "5" = "12h", "6" = "daily",
      ...
      geom_col(data = df_plot,
               ...
               colour = "white") + 
      scale_fill_manual("Continent", labels = c("AFr", "Asi", "Eur", "North", "South", "Golab"),
                        values=mycol_gridscale7)}
